/**
 * New Releases Page
 * Shows new releases from Steam's featured categories API.
 */
...
export default function NewReleases() {
  const { data, isLoading } = trpc.games.getNewReleases.useQuery(
    { cc: "us" },
    { staleTime: 5 * 60 * 1000 }
  );
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    "name": "New Game Releases on Steam",
    "description": "The latest new game releases on Steam, updated in real-time from Steam's official store.",
  ...
  return (
    <>
      <SEOHead
        title="New Game Releases on Steam | SteamPulse"
        description="Discover the latest new game releases on Steam. Updated in real-time from Steam's official store API."
        url="/new-releases"
        jsonLd={jsonLd}
      />
      ...
      <div className="container py-8 max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span>/</span>
            <span className="text-slate-300">New Releases</span>
          </div>
          <h1 className="text-3xl font-bold text-slate-100 mb-2">New Releases</h1>
          <p className="text-slate-400 max-w-2xl">
            The latest games released on Steam. Updated in real-time from Steam's official store.
          </p>
        </div>
            ...
            : games.map((game, idx) => (
              ...
              >
                <div className="relative">
                  ...
                  {/* New badge */}
                  <div className="absolute top-2 right-2 bg-indigo-500 rounded-md px-2 py-0.5 text-xs font-bold text-white">
                    NEW
                  </div>
                  {/* Discount badge */}
                  {game.discountPercent > 0 && (
                    <div className="absolute bottom-2 right-2 bg-emerald-500 rounded-md px-2 py-0.5 text-xs font-bold text-white">
        ...
        {!isLoading && games.length === 0 && (
          <div className="text-center py-16 text-slate-500">
            <p>No new releases data available right now. Please try again later.</p>
          </div>
        )}
        ...
        <div className="mt-8 rounded-xl border border-slate-700/30 bg-slate-800/20 p-4 text-sm text-slate-500">
          <p>
            New releases data is sourced directly from Steam's featured categories API.
            Visit{" "}
            <a href="https://store.steampowered.com/search/?filter=newreleases" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300">
              Steam Store
            </a>{" "}
